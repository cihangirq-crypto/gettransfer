...
// Supabase row'u DriverSession'a dönüştür
function mapRowToDriver(row: any): DriverSession {
  return {
    id: row.id,
    ...
    driverPerKm: row.driver_per_km || undefined,
    platformFeePercent: row.platform_fee_percent || undefined,
  }
}
...
async function sbUpsertDriver(d: DriverSession) {
  const data = {
    ...
    custom_pricing: d.customPricing || false,
    driver_per_km: d.driverPerKm || null,
    platform_fee_percent: d.platformFeePercent || null,
  }
...
export async function listDriversByStatus(status: 'approved' | 'pending' | 'rejected' | 'all'): Promise<DriverSession[]> {
  ...
  // Supabase çalışmıyorsa boş array döndür - DEMO VERİ KULLANMA!
  if (!Array.isArray(rows)) {
    console.error('Supabase not available for listing drivers with status:', status)
    return []
  }
  
  return rows.map(mapRowToDriver)
}
