...
router.post('/create', (req: Request, res: Response) => {
  ...
  try { (req.app.get('io') as any)?.emit('booking:create', booking) } catch {}
...
router.put('/:id/status', (req: Request, res: Response) => {
  ...
  bookings.set(id, b)
  try { (req.app.get('io') as any)?.emit('booking:update', b) } catch {}
...
router.put('/:id/route', (req: Request, res: Response) => {
  ...
  b.updatedAt = new Date().toISOString()
  bookings.set(id, b)
  try { (req.app.get('io') as any)?.emit('booking:route', { id: b.id, driverPath: driverPath }) } catch {}
  res.json({ success: true, data: b })
})
...
router.post('/:id/pay', (req: Request, res: Response) => {
  ...
  b.updatedAt = new Date().toISOString()
  bookings.set(id, b)
  try { (req.app.get('io') as any)?.emit('booking:paid', { id: b.id, amount: b.finalPrice }) } catch {}
  res.json({ success: true, data: b })
})
