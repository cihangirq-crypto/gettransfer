...
import { createBooking, generateReservationCode, getBookingById, updateBooking, listPendingBookings, getDriverEarnings, getDriverTripHistory, getPlatformAccounting } from '../services/bookingsStorage.js'
...
router.get('/requests', async (req: Request, res: Response) => {
  // TÜM pending talepleri getir - araç tipi filtresi YOK
  // Her sürücü tüm talepleri görebilir
  try {
    // Supabase'ten pending bookingleri çek (vehicleType filtresi yok)
    const pendingBookings = await listPendingBookings()
    ...
    const memoryList = Array.from(requests.values()).filter(r => r.status === 'pending')
...
router.post('/accept', (req: Request, res: Response) => {
  ...
  ;(async () => {
    ...
    if (!booking) {
      ...
      // Sürücünün özel fiyatlandırması var mı kontrol et
      let driverPerKm = pricing?.driverPerKm ?? 1
      let feePct = pricing?.platformFeePercent ?? 3
      let useCustomPricing = false
      
      if (driver && (driver as any).customPricing === true) {
        // Sürücüye özel fiyatlandırma aktif
        if (typeof (driver as any).driverPerKm === 'number' && (driver as any).driverPerKm > 0) {
          driverPerKm = (driver as any).driverPerKm
          useCustomPricing = true
        }
        if (typeof (driver as any).platformFeePercent === 'number' && (driver as any).platformFeePercent >= 0) {
          feePct = (driver as any).platformFeePercent
          useCustomPricing = true
        }
      }
      ...
      booking = await createBooking({
        ...
        extras: { 
          pricing: { 
            ...
            distanceKm: distKm, 
            driverFare, 
            platformFee: round2(total - driverFare), 
            total, 
            currency: pricing?.currency || 'EUR',
            customPricing: useCustomPricing
          } 
        },
      } as any)
    } else {
...
router.get('/earnings/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const earnings = await getDriverEarnings(req.params.id, { currency: pricing.currency })
    res.json({ success: true, data: earnings })
  } catch (e) {
    console.error('Failed to get driver earnings:', e)
    res.json({ success: true, data: { driverId: d.id, currency: 'EUR', daily: 0, weekly: 0, monthly: 0, total: 0, totalTrips: 0 } })
  }
})

// Sürücü yolculuk geçmişi
router.get('/trips/:id', async (req: Request, res: Response) => {
  ...
  try {
    const trips = await getDriverTripHistory(req.params.id, limit, offset)
    res.json({ success: true, data: trips })
  } catch (e) {
    console.error('Failed to get driver trips:', e)
    res.json({ success: true, data: [] })
  }
})

// Platform genel muhasebe
router.get('/accounting/platform', async (_req: Request, res: Response) => {
  try {
    const accounting = await getPlatformAccounting()
    res.json({ success: true, data: accounting })
  } catch (e) {
    console.error('Failed to get platform accounting:', e)
    res.json({ success: true, data: { 
      currency: 'EUR', 
      totalRevenue: 0, 
      totalDriverPayout: 0, 
      ...
      totalTrips: 0,
      daily: { revenue: 0, platformFee: 0 },
      weekly: { revenue: 0, platformFee: 0 },
      monthly: { revenue: 0, platformFee: 0 },
      dailyBreakdown: [],
      driverSummary: []
    }})
  }
