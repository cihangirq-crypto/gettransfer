...
export const useBookingStore = create<BookingState>()((set, get) => ({
  ...
  driverRefreshInterval: null as NodeJS.Timeout | null,
  ...
  startRealTimeUpdates: () => {
    const { lastRequestId, currentBooking, trackingLocation } = get();
    
    // Eğer aktif bir booking varsa, real-time dinle
    if (currentBooking?.id) {
      const channel = subscribeToBooking(currentBooking.id, (updatedBooking) => {
        set({ currentBooking: updatedBooking })
      })
      set({ realtimeChannel: channel })
      
      // Sürücü konumunu dinle
      const driverChannel = subscribeToDriverLocation(currentBooking.id, (location) => {
        // Sürücü konumunu store'da sakla
        const { currentBooking } = get()
        if (currentBooking) {
          set({
            currentBooking: {
      ...
      set({ driverLocationChannel: driverChannel })
    }
    
    // Polling fallback (Supabase realtime garantili çalışmayabilir)
    const pollingInterval = setInterval(async () => {
      const { lastRequestId, currentBooking } = get();
      
      // Eğer bekleyen bir talep varsa
      if (lastRequestId && !currentBooking?.driverId) {
        try {
          const res = await fetch(`${API}/bookings/${lastRequestId}`);
          const data = await res.json();
          if (data?.success && data?.data) {
            const booking = data.data;
            if (['accepted', 'driver_en_route', 'driver_arrived', 'in_progress'].includes(booking.status)) {
              set({ currentBooking: booking, availableDrivers: [] });
              
              // Real-time subscription başlat
              const channel = subscribeToBooking(booking.id, (updated) => {
                set({ currentBooking: updated })
              })
              set({ realtimeChannel: channel })
            }
          }
      ...
      // Aktif booking durumunu güncelle
      if (currentBooking?.id && ['accepted', 'driver_en_route', 'driver_arrived', 'in_progress'].includes(currentBooking.status)) {
        try {
    ...
    }, 2000); // 2 saniyede bir (daha hızlı)
    
    set({ updateInterval: pollingInterval as unknown as NodeJS.Timeout });
    
    // Sürücü listesini periyodik yenile (her zaman çalıştır)
    get().refreshApprovedDriversNear(trackingLocation || { lat: 41.0082, lng: 28.9784 }); // Default Istanbul
    const driverInterval = setInterval(() => {
      const loc = get().trackingLocation;
      if (loc) get().refreshApprovedDriversNear(loc);
    }, 5000); // Her 5 saniyede bir sürücüleri güncelle
    
    set({ driverRefreshInterval: driverInterval as unknown as NodeJS.Timeout });
  },
  ...
  stopRealTimeUpdates: () => {
    const { updateInterval, driverRefreshInterval, realtimeChannel, driverLocationChannel } = get();
    if (updateInterval) { clearInterval(updateInterval); set({ updateInterval: null }); }
    if (driverRefreshInterval) { clearInterval(driverRefreshInterval); set({ driverRefreshInterval: null }); }
    if (realtimeChannel) { unsubscribe(realtimeChannel); set({ realtimeChannel: null }); }
    if (driverLocationChannel) { unsubscribe(driverLocationChannel); set({ driverLocationChannel: null }); }
  },
